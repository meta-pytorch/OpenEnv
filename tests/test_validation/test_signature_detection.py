...
def test_the_table_lists_exactly_the_implemented_parsers():
    ...
    # Harbor parser (slice 6) and task.md with the PostTrain parser (slice F1).
...
def test_two_well_known_files_are_ambiguous_never_a_guess(tmp_path, monkeypatch):
    # Ambiguity needs two parseable formats, which this build doesn't have yet —
    # exercised against a patched table so the contract is enforced from day one.
    monkeypatch.setitem(WELL_KNOWN_FILES, SignatureKind.HARBOR_TASK, "task.toml")
