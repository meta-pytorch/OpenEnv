...
def test_checkpoint_1_walking_skeleton():
    ...
    The three checkpoint commands from structure.md §10, run through the CLI:
    a real env exits 0 with a schema-valid report; a broken manifest exits 1
    failing static.manifest; an ambiguous package exits 2.
    """
    ...
    unrecognized = runner.invoke(
        app,
        [
            "validate",
            str(FIXTURES / "unrecognized_package"),
            "--level",
            "static",
            "--skip-build",
        ],
    )
    assert unrecognized.exit_code == 2, unrecognized.output
