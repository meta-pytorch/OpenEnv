...
# One rollout (sandbox cold start + Node/Claude Code install + pi run + verifier) can
...
# Default Hugging Face sandbox image (cold-installs Node + Claude Code per rollout).
...
class ClaudeCodeEnvironment(MCPEnvironment):
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            from ..harness import ClaudeCodeSessionFactory
            from ..task import ClaudeCodeTask
        except ImportError:  # pragma: no cover
            from config import ClaudeCodeConfig  # type: ignore
            from harness import ClaudeCodeSessionFactory  # type: ignore
            from task import ClaudeCodeTask  # type: ignore
        ...
        self._ClaudeCodeState = ClaudeCodeState
        self._ClaudeCodeConfig = ClaudeCodeConfig
        self._ClaudeCodeSessionFactory = ClaudeCodeSessionFactory
        self._ClaudeCodeTask = ClaudeCodeTask
        ...
        # the result payload).
        self._state = self._ClaudeCodeState(episode_id=str(uuid4()))

        mcp = FastMCP("claude_code_env")
        ...
        def run_rollout(
            ...
            """Run one Claude Code rollout end-to-end.
            ...
            See ``claude_code_env.client.ClaudeCodeEnv.run_rollout`` for full arg docs.
    ...
    def reset(
        ...
        **_: Any,
        ...
        self._state = self._ClaudeCodeState(episode_id=episode_id or str(uuid4()))
        return Observation(
            done=False,
            reward=None,
            metadata={
                "status": "ready",
                "message": (
                    "claude_code_env ready. Call run_rollout(...) with a task."
                ),
            },
        )
    ...
    def _run_rollout_impl(
        ...
        *,
        ...
        # Build ClaudeCodeConfig + factory. We keep the proxy in charge of
        # ``model_override`` / ``logprobs`` / ``max_tokens``-cap injection.
        config = self._ClaudeCodeConfig(
        ...
        cc_task = self._ClaudeCodeTask(
            instruction=instruction_payload,
            metadata={"task_id": task_id},
        )

        factory = self._ClaudeCodeSessionFactory(
            config=config,
        ...
        try:
            _emit(
                ...
                "this is the slow phase (Node + Claude Code cold-install per rollout)"
            )
            session = factory.create(task=cc_task, start_agent=False)
