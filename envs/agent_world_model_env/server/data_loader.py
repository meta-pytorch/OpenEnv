...
class AWMDataLoader:
    ...
    def _build_verifiers(self, verifier_mode: str = "sql") -> dict[str, list[dict]]:
        ...
        if verifier_mode not in self._verifiers:
            ...
            self._verifiers[verifier_mode] = {}
            for item in raw:
                key = normalize_scenario_name(item["scenario"])
                if key not in self._verifiers[verifier_mode]:
                    self._verifiers[verifier_mode][key] = []
                self._verifiers[verifier_mode][key].append(item)
