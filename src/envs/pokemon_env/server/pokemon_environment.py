...
class PokemonEnvironment(Environment):
    ...
    def __init__(
        ...
        # Synchronization - SINGLE LOCK for both reset and step!
        self._env_lock = Lock()
    ...
    def _cleanup_old_battles(self):
        """Clean up old battles from player history to prevent memory leak."""
        if not self.player.battles:
            return

        # Keep only the most recent battle
        if len(self.player.battles) > 1:
            battles_to_remove = list(self.player.battles.keys())[:-1]  # All except last
            for battle_id in battles_to_remove:
                logger.debug(f"Cleaning up old battle: {battle_id}")
                del self.player.battles[battle_id]

            logger.info(f"Cleaned up {len(battles_to_remove)} old battles. Current count: {len(self.player.battles)}")

    def _cancel_previous_battle(self):
        """Cancel any running battle task from previous episode."""
        if self._battle_task and not self._battle_task.done():
            logger.warning("Cancelling previous battle task")
