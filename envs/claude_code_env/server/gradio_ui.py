...
"""Minimal Gradio UI for claude_code_env.
...
PRESETS: dict[str, dict[str, str]] = {
    ...
    "fizzbuzz": {
        ...
        "setup": "",
        "verify": (
            "test -f /root/workdir/fizzbuzz.py\n"
            "python -c \"import sys; sys.path.insert(0, '/root/workdir'); "
            "import fizzbuzz; "
    ...
    "pandas_csv": {
        "instruction": (
            "Read `/root/data/numbers.csv` (a CSV with a single column "
            "`x` of integers) using pandas. Compute the mean of the `x` "
            "column and write it as a single float to `/root/workdir/mean.txt` "
        ...
        "verify": (
            "test -f /root/workdir/mean.txt\n"
            "python -c \"v=float(open('/root/workdir/mean.txt').read().strip()); "
...
def pi_gradio_builder(
    ...
    """Build the claude_code_env console.

    Compatible with ``create_app(..., gradio_builder=...)``. We ignore
    ``web_manager`` and instantiate :class:`ClaudeCodeEnvironment` ourselves inside
    the run handler — claude_code_env's run_rollout doesn't need any per-session
    state beyond the env's own bookkeeping, and instantiating is cheap (no
    sandbox is created until the tool fires).
    """
    ...
    with gr.Blocks(title=title or "claude_code_env") as app:
        gr.Markdown(f"# {title or 'claude_code_env'}")
        gr.Markdown(
            "Run one Claude Code rollout in a Hugging Face sandbox against your chosen "
        ...
        instruction = gr.Textbox(
            label="Instruction (the prompt pi runs)",
            lines=4,
