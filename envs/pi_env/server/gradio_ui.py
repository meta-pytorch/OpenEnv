...
def pi_gradio_builder(
    ...
    """Build the pi_env console.

    Compatible with ``create_app(..., gradio_builder=...)``. We ignore
    ``web_manager`` and instantiate :class:`PiEnvironment` ourselves inside
    the run handler — pi_env's run_rollout doesn't need any per-session
    ...
    with gr.Blocks(title=title or "pi_env") as app:
        gr.Markdown(f"# {title or 'pi_env'}")
        gr.Markdown(
            "Run one Pi rollout in a Hugging Face sandbox against your chosen "
