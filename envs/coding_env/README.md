...
PYTHONPATH=src:envs uv run pytest tests/envs/test_python_codeact_reset.py tests/envs/test_python_codeact_rewards.py -v

# Run integration tests (requires Docker image to be built)
docker build -t coding-env:latest -f envs/coding_env/server/Dockerfile .
SKIP_DOCKER_TESTS=0 PYTHONPATH=src:envs uv run pytest tests/envs/test_coding_env_integration.py -v
```
