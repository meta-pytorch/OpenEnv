...
"""Pydantic models for the deployed pi_env HTTP server.
...
config, runs one Pi rollout end-to-end inside a Hugging Face sandbox, and
...
class PiState(State):
    """Per-session env state across calls to one PiEnvironment instance.
