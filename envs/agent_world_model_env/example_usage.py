...
async def main():
    async with AWMEnv(base_url="http://localhost:8899") as env:
        # get list of available environments (scenarios)
        ...
        # select a environment (scenario) and reset it
        # Reset returns verifier support info (has_verifier: {sql: bool, code: bool} or None)
        # Pass LLM credentials for sql verifier mode (or set via OPENENV_AWM_LLM_* env vars on server)
        ...
        print(
            "reset result:",
            f"scenario: {result.observation.scenario}, task: {task_description}, has_verifier: {result.observation.has_verifier}, total tools: {result.observation.num_tools}",
        )
        print("=" * 100)
        ...
        # =====================================================================
        # Agent loop
        # please set the llm credentials by exporting ENDPOINT_URL, AZURE_OPENAI_API_KEY
        # =====================================================================
        print("=" * 100)
        ...
        for step in range(1, MAX_ITERATIONS + 1):
            ...
            print(
                f"[Step {step}] Tool call: {name} {json.dumps(arguments, ensure_ascii=False)[:200]}"
            )
        ...
        # Verification: call verify tool with verifier_mode (can be "sql" or "code")
        ...
        # End episode: done only destroys the environment (does NOT run verifier)
        result: StepResult[AWMObservation] = await env.step(
...
if __name__ == "__main__":
    # please manually start the environment server first
    # if you want to use sql verifier mode, you should export the environment variables
    # OPENENV_AWM_LLM_BASE_URL, OPENENV_AWM_LLM_API_KEY, OPENENV_AWM_LLM_MODEL
    # uv run uvicorn agent_world_model_env.server.app:app --host 127.0.0.1 --port 8899
