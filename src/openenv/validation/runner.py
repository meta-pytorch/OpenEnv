...
from .graders.static_ import StaticManifestGrader
...
_DIGEST_EXCLUDED_DIRS = {".git", "__pycache__", ".venv", ".worktrees"}
...
def run_validation(
    ...
    *,
    ...
    Raises [`~openenv.validation.SignatureError`] for ambiguous/unrecognized packages
    and [`~openenv.validation.UnsupportedPackageError`] for recognized-but-unsupported
    ones (CLI exit code 2). A package whose declarations fail the manifest schema
    yields a normal report with a `static.manifest` FAIL (exit code 1).
    ...
    Args:
        ...
        provider ([`~openenv.validation.ValidationProvider`], *optional*):
        ...
        policy ([`~openenv.validation.SeverityPolicy`], *optional*):
            Severity policy; `None` loads the committed default version.

    Returns:
        [`~openenv.validation.ValidationReport`]: the completed report.
    """
    target = Path(target)
