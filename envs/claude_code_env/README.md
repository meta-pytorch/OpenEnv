...
`claude_code_env` runs the [Claude Code](https://github.com/badlogic/pi-mono) agent
...
agent is Claude Code instead of OpenCode, and the default sandbox backend is Hugging
Face instead of E2B.
...
what loop-owning GRPO training uses. The primitive uses the sandbox backend +
proxy from `openenv.core.sandbox`:
...
Claude Code is pointed at the endpoint via a `models.json` provider block written under
...
`HFSandboxBackend` (from `openenv.core.sandbox`)
runs the agent in a Hugging Face sandbox. `image="python:3.12"` cold-installs
Node 22 (bootstrapped) + the Claude Code CLI (`npm install -g @mariozechner/pi-coding-agent`)
+ the proxy's Python deps on every rollout. For faster rollouts use the
pre-baked image (Node + Claude Code + proxy deps already installed), built by CI from
`hf_image/Dockerfile`:
...
| `image` | `str` | `""` | HF sandbox image; blank → `python:3.12` (cold-installs Node + Claude Code). |
...
|---|---|---|
| **`transparent_proxy`** (default) | In-sandbox proxy at `localhost:7000` forwards Claude Code's LLM calls to `base_url`, injects `logprobs=true`, captures per-turn `(messages, completion_tokens, logprobs)` to `proxy_trace.jsonl`. | GRPO / RL training, observability, top-k distillation. |
| **`black_box`** | No proxy. Claude Code talks straight to `base_url`. | Smoke tests, eval, SFT data collection. |
...
├── claude_code_runtime.py                   # models.json builder + install/run cmds
...
`openenv.core.sandbox`; `claude_code_env` ships no `sandbox/` of its own.
