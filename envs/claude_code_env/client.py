...
"""Client for the deployed claude_code_env server.

The server exposes a single MCP tool ``run_rollout`` that runs one Claude Code
rollout in a Hugging Face sandbox and returns a JSON-serialized
:class:`RolloutResult`.
...
class ClaudeCodeEnv(MCPToolClient):
    """Typed client for the claude_code_env MCP server.
    ...
    def run_rollout(
        ...
        *,
        ...
        """Run one Claude Code rollout and return the typed result.
        ...
        Args:
            ...
            max_tokens_cap: Clamp on per-turn ``max_tokens``. gpt-4o-mini
                caps at 16k; keep this under the endpoint's limit.
            top_logprobs: Top-k logprobs requested upstream. HF Router caps
                at 5; OpenAI accepts up to 20; vLLM is unbounded.
            agent_timeout_s: Hard wall-clock budget for one ``pi`` run.
            image: Hugging Face sandbox image. Empty string uses the default
                (``python:3.12``, which cold-installs Node + Claude Code per rollout).
