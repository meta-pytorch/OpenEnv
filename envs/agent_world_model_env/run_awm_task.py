...
async def run_task(scenario: str, task_idx: int):
    print(f"\n{'#'*80}")
...
async def main():
    scenario = sys.argv[1] if len(sys.argv) > 1 else "workflow_automation_1"
    task_indices = range(10)

    results = []
    for task_idx in task_indices:
        try:
            r = await run_task(scenario, task_idx)
            results.append(r)
        except Exception as e:
            print(f"  ERROR on {scenario} task {task_idx}: {e}")
            results.append({
                "scenario": scenario,
                "task_idx": task_idx,
                "task": "",
    ...
    with open(f"awm_{scenario}_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nDetailed results saved to awm_{scenario}_results.json")
